      ...
      <div className="container mx-auto px-6 py-8 max-w-4xl">
        ...
        {loading && (
          <div className="space-y-6">
            {[1, 2, 3].map((i) => (
              <div key={i} className="animate-pulse bg-white h-20 rounded-lg shadow-sm ring-1 ring-gray-200" />
            ))}
          </div>
        )}
        ...
        {!loading && !error && (
          <>
            {quizzes.length === 0 ? (
              <Card className="shadow-sm ring-1 ring-gray-200 text-center py-12">
                <CardContent>
                  <div className="flex items-center justify-center w-12 h-12 rounded-lg bg-gray-100 ring-1 ring-gray-200 mx-auto mb-6">
                    <FileQuestion className="w-6 h-6 text-gray-700" />
                  </div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-2">No quizzes yet</h3>
                  <p className="text-gray-600 mb-6">Generate your first quiz to test your knowledge</p>
                  <Button
                    onClick={handleGenerate}
                    className="bg-gray-900 hover:bg-gray-800 cursor-pointer"
                  >
              ...
              <div className="space-y-6">
                {quizzes.map((q) => (
                  <Card key={q.id} className="shadow-sm ring-1 ring-gray-200 hover:shadow-md transition-all duration-200">
                    <CardContent className="p-6">
